      ...
      if (phones.length > 0 || contactIds.length > 0) {
        const result = await pool.query(
          ...
          `SELECT lead_phone, hubspot_contact_id,
           ...
           WHERE (lead_phone = ANY($1) OR hubspot_contact_id = ANY($2)) AND is_internal IS NOT TRUE AND status = 'completed'
  ...
  try {
    ...
    const calls = await pool.query(
      `SELECT id, created_at, caller_identity, duration_seconds, disposition,
       ...
       FROM nucleus_phone_calls
       WHERE (hubspot_contact_id = $1 OR lead_phone = $2) AND is_internal IS NOT TRUE AND status = 'completed'
